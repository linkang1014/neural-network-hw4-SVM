...
% save('result_task3.mat','train_data_mean','train_data_std','w','b');

%% Evaluation
load('result_task3.mat');
load('eval.mat');
%Preprocessing
%Standardization
for i = 1:57
    for j = 1:600
        eval_data(i,j) = (eval_data(i,j)-train_data_mean(i))./train_data_std(i);
    end
end
eval_predicted = sign(eval_data'*w+b);
Correct_eval_No = 0;
for i = 1:600
    if (eval_label(i)==1)&&(eval_predicted(i)==1)||(eval_label(i)==-1)&&(eval_predicted(i)==-1)
        Correct_eval_No = Correct_eval_No+1;
    end
end
Accuracy_eval = Correct_eval_No/600;
