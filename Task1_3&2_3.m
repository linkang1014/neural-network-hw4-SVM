...
for i = 1:2000
    for j = 1:2000
%         H(i,j) = train_label(i)*train_label(j)*(train_data(:,i)'*train_data(:,j)+1)^p(t);
        K(i,j) = (train_data(:,i)'*train_data(:,j)+1)^p(t);
    end
end
Mercer_condition = eig(K);
Positive_semi_definite = ones(1,5);
for i = 1:2000
    if Mercer_condition(i)<0
        Positive_semi_definite(t) = 0;
    end
end
